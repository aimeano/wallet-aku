...
export const TransactionList = ({ transactions, categories, empty, onSelect, onDelete }: Props) => {
      ...
      {transactions.map((t, i) => {
        ...
        const row = (
          <button
            ...
            onClick={() => onSelect?.(t)}
            disabled={!onSelect}
            className="group flex w-full items-center gap-3 rounded-2xl border border-border/60 bg-gradient-card p-3 text-left shadow-card transition-smooth enabled:hover:border-border enabled:active:scale-[0.99] disabled:cursor-default"
          >
            <div
        ...
        const swipeable = onSelect || onDelete;
        ...
        return (
          ...
          >
            {swipeable ? (
              <SwipeableRow
                onSwipeRight={onSelect ? () => onSelect(t) : undefined}
                onSwipeLeft={onDelete ? () => onDelete(t) : undefined}
              >
                {row}
              </SwipeableRow>
            ) : row}
